...
class WorldModelVICReg(nn.Module):
    def __init__(self, loss_type='vicreg', lambda_param=25.0, mu_param=25.0, nu_param=1.0):
        super().__init__()
    ...
    def training_step(self, batch):
        ...
        accumulated_losses = {
            'total_loss': 0.0,
            'sim_loss' if self.loss_type == 'vicreg' else 'on_diag_loss': 0.0,
            'std_loss' if self.loss_type == 'vicreg' else 'off_diag_loss': 0.0,
            'cov_loss' if self.loss_type == 'vicreg' else 'total_loss': 0.0
        }
